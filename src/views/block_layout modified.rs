...
pub struct BaseLayout {
    ...
    pub right_idx: usize,
    pub layers: Vec<f64>,
    pub node_layers: HashMap<GraphNode, i64>,
...
impl BaseLayout {
    ...
    /// Initialize the layers and node_layers fields for the first partition
    fn initialize_layers(&mut self) {
        // Get and sort all unique x-coordinates
        self.layers = self.node_positions.values()
            .map(|(x, _)| *x)
            .sorted_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal))
            .dedup_by(|a, b| (a - b).abs() < f64::EPSILON)
            .collect();
        
        // Map each node to its layer index
        for (node, (x, _)) in &self.node_positions {
            let layer_index = self.layers.iter()
                .position(|layer_x| (layer_x - x).abs() < f64::EPSILON)
                .unwrap_or(0) as i64;
            
            self.node_layers.insert(*node, layer_index);
        }
    }

    /// Add new layers to the left or right of the existing layers
    fn update_layers_for_expansion(&mut self, new_node_positions: &[(GraphNode, (f64, f64))], rightwards: bool) {
        if new_node_positions.is_empty() || self.layers.is_empty() {
            return;
        }

        if rightwards {
            // For rightward expansion, all layers with x-coordinates greater than 
            // the maximum existing layer are new
            let max_x = *self.layers.last().unwrap();
            
            // Extract and sort unique x-coordinates from the new nodes that are greater than max_x
            let new_layers: Vec<f64> = new_node_positions.iter()
                .map(|(_, (x, _))| *x)
                .filter(|x| (*x - max_x).abs() > f64::EPSILON && *x > max_x)
                .sorted_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal))
                .dedup_by(|a, b| (a - b).abs() < f64::EPSILON)
                .collect();
            ...
            // Map new nodes to their layer indices
            for (node, (x, _)) in new_node_positions {
                let layer_index = self.layers.iter()
                    .position(|layer_x| (layer_x - x).abs() < f64::EPSILON)
                    .unwrap_or(0) as i64;
            ...
            // For leftward expansion, all layers with x-coordinates less than 
            // the minimum existing layer are new
            let min_x = self.layers[0];
            
            // Extract and sort unique x-coordinates from the new nodes that are less than min_x
            let new_layers: Vec<f64> = new_node_positions.iter()
                .map(|(_, (x, _))| *x)
                .filter(|x| (*x - min_x).abs() > f64::EPSILON && *x < min_x)
                .sorted_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal))
                .dedup_by(|a, b| (a - b).abs() < f64::EPSILON)
                .collect();
            
            // Calculate layer offset (number of new layers)
            let layer_offset = new_layers.len() as i64;
            
            // Prepend new layers to the front
            if !new_layers.is_empty() {
                self.layers = new_layers.into_iter()
                    .chain(self.layers.iter().copied())
                    .collect();
                
                // Shift existing node layer indices by the number of new layers
                for layer_idx in self.node_layers.values_mut() {
            ...
            // Map new nodes to their layer indices
            for (node, (x, _)) in new_node_positions {
                // Find the index of the layer this node belongs to
                let layer_index = self.layers.iter()
                    .position(|layer_x| (layer_x - x).abs() < f64::EPSILON)
                    .unwrap_or(0) as i64;
    ...
    fn expand(&mut self, rightwards: bool) {
        ...
        let new_positions = if rightwards {
            new_node_positions.iter().map(|(node, pos)| {
        ...
        // Add the new positions to the node_positions hashmap
        for (node, pos) in &new_positions {
        ...
        // Update the layers incrementally
        //self.update_layers_for_expansion(&new_positions, rightwards);
...
impl ScaledLayout {
    ...
    pub fn refresh(&mut self, base_layout: &BaseLayout, parameters: &PlotParameters) {
        ...
        if parameters.label_width < 5 {
            ...
            // Use the pre-computed layer information from BaseLayout
            let mut cumulative_offsets = Vec::with_capacity(base_layout.layers.len());
            
            // First determine the width of each layer based on node label widths
            let mut layer_widths = vec![1; base_layout.layers.len()]; // Initialize all with minimum width
            
            // Find maximum label width for each layer
            for (node, _) in &working_layout {
                let label_width = std::cmp::min(
                    node.sequence_end - node.sequence_start,
                    parameters.label_width as i64,
                );
                
                if let Some(&idx) = base_layout.node_layers.get(node) {
                    layer_widths[idx as usize] = std::cmp::max(layer_widths[idx as usize], label_width);
                }
            }
            ...
            // Position each node using its layer information
            for (node, (_, y)) in &working_layout {
                if let Some(&idx) = base_layout.node_layers.get(node) {
                    let label_width = std::cmp::min(
                        node.sequence_end - node.sequence_start,
                        parameters.label_width as i64,
                    ...
                    let layer_width = layer_widths[idx as usize];
                    let x_offset = cumulative_offsets[idx as usize];
                    
                    // Center the node in its layer
                    let centering_offset = ((layer_width - label_width) as f64 / 2.0).round() as i64;
                    let x = x_offset + centering_offset;
