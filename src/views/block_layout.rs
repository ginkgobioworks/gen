...
impl ScaledLayout {
    ...
    pub fn rescale(&mut self, parameters: &PlotParameters) {
        ...
        let layout: Vec<(u32, (u32, u32))> = self
            ._raw_layout
            .iter()
            .map(|(id, (x, y))| {
                (
                    *id,
                    ((y * scale_x).round() as u32, (x * scale_y).round() as u32),
                )
            })
        ...
        if self._sequences.is_none() || (parameters.label_width < 5) {
            ...
            let layout: HashMap<u32, (u32, u32)> =
                layout.iter().map(|(id, (x, y))| (*id, (*x, *y))).collect();
            self.lines = self
                ._edges
                .iter()
                .map(|(source, target)| {
                    let source_coord = layout
                        .get(source)
                        .map(|&(x, y)| (x as f64 + 0.5, y as f64 + 0.25))
                        .unwrap();
                    let target_coord = layout
                        .get(target)
                        .map(|&(x, y)| (x as f64 - 1.0, y as f64 + 0.25))
                        .unwrap();
        ...
        let mut layer_width = std::cmp::min(
            self._sequences
                .as_ref()
                .unwrap()
                .get(&layout[0].0)
...
mod tests {
    ...
    fn test_scaled_layout_new_edges() {
        ...
        let edges = vec![(0, 1)];
        let raw_layout = vec![(0, (5.0, 0.0)), (1, (5.0, 10.0))];
        let full_labels = HashMap::from([(0, "ABCDEFGH".to_string()), (1, "IJKLMNOP".to_string())]);
