...
impl Query for PathEdge {
    ...
    fn process_row(row: &Row) -> Self::Model {
        PathEdge {
            ...
            block_group_edge_id: row.get(3).unwrap(),
        }
    }
}
...
impl PathEdge {
    pub fn create(
        ...
        path_id: i64,
        index_in_path: i64,
        block_group_edge_id: i64,
    ) -> PathEdge {
        let query =
            "INSERT INTO path_edges (path_id, index_in_path, block_group_edge_id) VALUES (?1, ?2, ?3) RETURNING (id)";
        let mut stmt = conn.prepare(query).unwrap();
        let mut rows = stmt
            .query_map((path_id, index_in_path, block_group_edge_id), |row| {
                Ok(PathEdge {
                    id: row.get(0)?,
                    path_id,
                    index_in_path,
        ...
        match rows.next().unwrap() {
            ...
            Err(rusqlite::Error::SqliteFailure(err, _details)) => {
                if err.code == rusqlite::ErrorCode::ConstraintViolation {
                    ...
                    let query = "SELECT id from path_edges where path_id = ?1 AND block_group_edge_id = ?2;";
                    placeholders.push(block_group_edge_id);
    ...
    pub fn bulk_create(conn: &Connection, path_id: i64, block_group_edge_ids: &[i64]) {
        for (index1, chunk) in block_group_edge_ids.chunks(100000).enumerate() {
            let mut rows_to_insert = vec![];
            for (index2, block_group_edge_id) in chunk.iter().enumerate() {
                let row = format!(
                    "({0}, {1}, {2})",
                    path_id,
                    block_group_edge_id,
            ...
            let insert_statement = format!(
                "INSERT OR IGNORE INTO path_edges (path_id, block_group_edge_id, index_in_path) VALUES {0};",
    ...
    pub fn edges_for_path(conn: &Connection, path_id: i64) -> Vec<Edge> {
        ...
        let block_group_edge_ids = path_edges
            .into_iter()
            .map(|path_edge| path_edge.block_group_edge_id)
            .collect::<Vec<i64>>();
    ...
    pub fn block_group_edges_for_path(conn: &Connection, path_id: i64) -> Vec<BlockGroupEdge> {
        let path_edges = PathEdge::query(
            conn,
            "select * from path_edges where path_id = ?1 order by index_in_path ASC",
            rusqlite::params!(Value::from(path_id)),
        ...
        block_group_edge_ids
            .into_iter()
            .map(|block_group_edge_id| block_group_edges_by_id[&block_group_edge_id].clone())
            .collect::<Vec<BlockGroupEdge>>()
    }
    ...
    pub fn edges_for_paths(conn: &Connection, path_ids: Vec<i64>) -> HashMap<i64, Vec<Edge>> {
        ...
        let block_group_edge_ids = path_edges
            ...
            .collect::<Vec<i64>>();
        let block_group_edges = BlockGroupEdge::load_block_group_edges(conn, &block_group_edge_ids);
        let edge_ids_by_block_group_edge_id = block_group_edges
            .clone()
            .into_iter()
            .map(|block_group_edge| (block_group_edge.id, block_group_edge.edge_id))
        ...
        let path_edges_by_path_id = path_edges
            ...
            .map(|path_edge| {
                (
                    path_edge.path_id,
                    edge_ids_by_block_group_edge_id[&path_edge.block_group_edge_id],
                )
            })
            .into_group_map();
